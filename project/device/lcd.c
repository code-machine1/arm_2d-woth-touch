...
void LCD_ShowFloatNum(u16 x, u16 y, float num, u8 float_n, uint16_t fc, uint16_t bc, u8 size)
{
    ...
    LCD_ShowIntNum(x, y, zhengshu, zhengshu_counter, fc, bc, size); //显示整数部分
    LCD_ShowChar((x + size * zhengshu_counter / 2), y, '.', fc, bc, size, 0);   //显示小数点
    LCD_ShowIntNum((x + size * zhengshu_counter / 2 + size / 2), y, xiaoshu, float_n, fc, bc, size); //显示整数部分
}
...
char string[20];
static void insertChar(char *str, char ch, int pos)
{
    int len = strlen(str);
